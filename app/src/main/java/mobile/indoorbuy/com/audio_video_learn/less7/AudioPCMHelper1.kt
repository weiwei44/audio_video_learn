 ...
 * 实时录音，保存aac,完善版本
 */
class AudioPCMHelper1 : Runnable {
        ...
        try {
            while (isPlaying) {
                ...
                if (inputIndex >= 0) {
                    ...
                    val readSize = extractor.readSampleData(inputByteBuf, 0)
                    if (readSize < 0) {
                        Log.e("weiwei","over")
                    ...
                    mMediaCodec!!.queueInputBuffer(inputIndex, 0, readSize, presentationTimeUs, 0)
                    extractor.advance()
                }
